...
local B = {
    ...
    Bloodlust    = 2825,     -- 嗜血 — 部落萨满技能，+30%急速
    Heroism      = 32182,    -- 英勇/英雄主义 — 联盟萨满/猎人技能，+30%急速
}
...
local function APLCallback()
    ...
    if moonChosenDur > 0 and castingSpellName ~= "月神轨道炮" then
        ...
        -- 有嗜血时优先星火（吃急速收益高），无嗜血时优先愤怒
        if hasBloodlust then
            -- 嗜血期间：智能分配星火和愤怒
            -- 策略：先打星火消耗苍穹之焰，剩余6-7秒时切换愤怒打完星辰之怒
            local timeRemaining = moonChosenDur
            local wrathTimeNeeded = wStacks * wrathCT  -- 打完所有愤怒需要的时间
            
            -- 如果剩余时间接近结束（6-7秒），且还有星辰之怒层数，优先打愤怒
            ...
            elseif wStacks > 0 then
                ...
                -- 都耗尽后，根据自然之赐判断
                if (natureGrace>math.min(starfireCT,2*wrathCT) or (natureGrace>0 and natureGrace<wrathCT)) or wrathCT < 0.75 then
                    return S.Starfire
                end
                return S.Wrath
            end
        else
            -- 无嗜血时：优先愤怒消耗星辰之怒，其次星火消耗苍穹之焰
            if wStacks > 0 then
                return S.Wrath
    ...
    and ((targetDeadTime <= 20 or targetDeadTime > 20 + eclipseDur - railgunCT)
        ...
        -- Boss剩余时间<10秒，不开炮，保持月蚀打满
        if targetDeadTime > 0 and targetDeadTime < 10 then
            return 6603  -- 等待，用填充技能打完月蚀
        end
        
        -- 智能延迟开炮：避免最后出现疲软期
        local moonChosenRemaining = math.max(0, VF_getBuff("player", B.MoonChosen, "HELPFUL|PLAYER") - GCD)
        local canCastMultipleRailguns = (targetDeadTime > railgunCD + railgunCT + 20)  -- 目标存活 > 85s 才能覆盖2次炮（CD60+读条5+buff20）
        
        if not canCastMultipleRailguns then
            -- 只能释放一次轨道炮时，计算是否能覆盖到战斗结束
            local timeAfterRailgun = eclipseDur - railgunCT  -- 开完炮后月蚀剩余时间
            local totalBuffTime = 20 + railgunCT  -- 月神选民20s + 轨道炮读条5s = 25s
            local bufferTime = 2  -- 缓冲时间2秒
            
            -- 如果现在开炮会导致最后有疲软期（月神选民结束后还有很长时间），则延迟
            -- 理想情况：在月蚀快结束时开炮，让月神选民覆盖到最后
            if timeAfterRailgun > bufferTime and targetDeadTime > totalBuffTime then
                -- 月蚀还很久，且战斗时间足够长，延迟到月蚀剩~bufferTime秒时再开炮
                return 6603
            end
        end
    ...
    -- P5: 月蚀 + 炮 CD 中/击杀时间在月蚀与选民微妙的节奏中 调控节奏
    if eclipseDur > 0 and (railgunCD > CastWindow or (targetDeadTime > 20 and targetDeadTime <= 20 + eclipseDur - railgunCT)) then
        ...
        -- 双满兜底 → 愤怒>0.75秒优先愤怒，否则根据自然之赐判断
        if wrathCT > 0.75 then
            return S.Wrath
        end
        if (natureGrace>math.min(starfireCT,2*wrathCT) or (natureGrace>0 and natureGrace<wrathCT)) then
            return S.Starfire
        end
        return S.Wrath
    end
